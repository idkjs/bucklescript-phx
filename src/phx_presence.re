...
type onJoinOrLeave =
  (~key: string=?, ~currentPresence: state=?, ~newPresence: state=?, unit) =>
  void;
...
external syncState:
  (state, state, ~onJoin: onJoinOrLeave=?, ~onLeave: onJoinOrLeave=?, unit) =>
  state =
  "syncState";
...
[@bs.scope "Presence"] [@bs.module "phoenix"]
external syncDiff:
  (state, diff, ~onJoin: onJoinOrLeave=?, ~onLeave: onJoinOrLeave=?, unit) =>
  state =
  "syncDiff";
...
[@bs.scope "Presence"] [@bs.module "phoenix"]
external list: (any, ~chooser: chooser=?, unit) => any = "list";
let list = (~chooser=?, presences) => list(presences, ~chooser?, ());

[@bs.send]
external onState: (Channel.t, string, state => void, unit) => void = "on";
let onState = (f, channel) => {
  let _ = onState(channel, "presence_state", f, ());
  channel;
};
[@bs.send]
external onDiff: (Channel.t, string, diff => void, unit) => void = "on";
